    ...
    <aside className={cn('sidebar flex flex-col', collapsed && 'collapsed')}>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-1">
        {filteredNav.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={18} className={cn('flex-shrink-0', isActive ? 'text-blue-600' : 'text-slate-500 group-hover:text-slate-800')} />
              {!collapsed && <span>{item.label}</span>}
              {!collapsed && isActive && <ChevronRight size={14} className="ml-auto text-blue-600/70" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="p-3 border-t border-slate-200 space-y-2">
        ...
        <Link
          href="/"
          className={cn(
            'flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 transition-all duration-200',
            collapsed && 'justify-center px-2'
          )}
      ...
      <button
        ...
        className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors z-10 text-slate-600 shadow-xs"
